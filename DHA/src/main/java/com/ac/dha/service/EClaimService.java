...
import com.ac.dha.entties.SearchTransactionsRequest;
...
public class EClaimService {
	...
	public ResponseEntity<String> searchTransactions(SearchTransactionsRequestDTO dto) {
		...
		try {
			// Save request entity for auditing
			...
			log.info("Saved SearchTransactionsRequest with ID: {}", requestEntity.getId());

			// Convert DTO to XML
			byte[] xmlPayload = xmlUtil.convertToXml(dto);
			log.info("XML Payload: {}", new String(xmlPayload));
			...
			log.info("Parsed UploadERxRequestResponseDTO: {}", tempResponse);

			// Map to SearchTransactionsResponseDTO if needed
			SearchTransactionsResponseDTO responseDTO = new SearchTransactionsResponseDTO();
			responseDTO.setFoundTransactions(tempResponse.geteRxReferenceNo() != null ? "1" : "0"); // Example mapping
			...
			try {
				...
				return ResponseEntity.status(500).contentType(MediaType.APPLICATION_JSON).body(jsonError);
			} catch (Exception jsonException) {
				log.error("Error serializing error response: {}", jsonException.getMessage(), jsonException);
				return ResponseEntity.status(500).contentType(MediaType.APPLICATION_JSON)
						.body("{\"foundTransactions\":\"0\",\"errorMessage\":\"Internal server error\"}");
			}
		}
	}
